...
public class RuntimeManager {
    ...
    private RuntimeManagerData data = new RuntimeManagerData(new CopyOnWriteArrayList<DataSourceRT>(), new ConcurrentHashMap<Integer, DataPointRT>(), new ConcurrentHashMap<Integer, DataPointListener>(), new ConcurrentHashMap<String, SimpleEventDetector>(),
			new ConcurrentHashMap<Integer, CompoundEventDetectorRT>(), new CopyOnWriteArrayList<PublisherRT<?>>(), new CopyOnWriteArrayList<PointLinkRT>(), new CopyOnWriteArrayList<MaintenanceEventRT>(), false);
    ...
    synchronized public void terminate() {
        ...
        data.started = false;
        for (MaintenanceEventRT me : data.maintenanceEvents)
            stopMaintenanceEvent(me.getVo().getId());

        for (PublisherRT<? extends PublishedPointVO> publisher : data.runningPublishers)
            stopPublisher(publisher.getId());

        for (Integer id : data.compoundEventDetectors.keySet())
            stopCompoundEventDetector(id);

        for (PointLinkRT pointLink : data.pointLinks)
            stopPointLink(pointLink.getId());

        // First stop meta data sources.
        for (DataSourceRT dataSource : data.runningDataSources) {
        ...
        for (String key : data.simpleEventDetectors.keySet())
    ...
    private boolean initializeDataSource(DataSourceVO<?> vo) {
        synchronized (data.runningDataSources) {
            ...
            data.runningDataSources.add(dataSource);
    ...
    private void stopDataSource(int id) {
        synchronized (data.runningDataSources) {
            ...
            for (DataPointRT p : data.dataPoints.values()) {
                if (p.getDataSourceId() == id)
                    stopDataPoint(p.getId());
            }

            data.runningDataSources.remove(dataSource);
    ...
    private void startDataPoint(DataPointVO vo) {
        synchronized (data.dataPoints) {
            ...
            if (ds != null) {
                ...
                data.dataPoints.put(dataPoint.getId(), dataPoint);
    ...
    private void stopDataPoint(int dataPointId) {
        synchronized (data.dataPoints) {
            ...
            DataPointRT p = data.dataPoints.remove(dataPointId);
    ...
    public boolean isDataPointRunning(int dataPointId) {
        return data.dataPoints.get(dataPointId) != null;
    }

    public DataPointRT getDataPoint(int dataPointId) {
        return data.dataPoints.get(dataPointId);
    }

    public void addDataPointListener(int dataPointId, DataPointListener l) {
        DataPointListener listeners = data.dataPointListeners.get(dataPointId);
        data.dataPointListeners.put(dataPointId, DataPointEventMulticaster.add(listeners, l));
    }

    public void removeDataPointListener(int dataPointId, DataPointListener l) {
        DataPointListener listeners = DataPointEventMulticaster.remove(data.dataPointListeners.get(dataPointId), l);
        if (listeners == null)
            data.dataPointListeners.remove(dataPointId);
        else
            data.dataPointListeners.put(dataPointId, listeners);
    }

    public DataPointListener getDataPointListeners(int dataPointId) {
        return data.dataPointListeners.get(dataPointId);
    }
    ...
    public void setDataPointValue(int dataPointId, PointValueTime valueTime, SetPointSource source) {
        DataPointRT dataPoint = data.dataPoints.get(dataPointId);
        if (dataPoint == null)
    ...
    public long purgeDataPointValues() {
        ...
        for (Integer id : data.dataPoints.keySet())
    ...
    private void startScheduledEvent(ScheduledEventVO vo) {
        synchronized (data.simpleEventDetectors) {
            stopSimpleEventDetector(vo.getEventDetectorKey());
            ScheduledEventRT rt = vo.createRuntime();
            data.simpleEventDetectors.put(vo.getEventDetectorKey(), rt);
            rt.initialize();
        }
    }
    ...
    public void stopSimpleEventDetector(String key) {
        synchronized (data.simpleEventDetectors) {
            SimpleEventDetector rt = data.simpleEventDetectors.remove(key);
            if (rt != null)
                rt.terminate();
        }
    }
    ...
    public void addPointEventDetector(PointEventDetectorRT ped) {
        synchronized (data.simpleEventDetectors) {
            ped.initialize();
            data.simpleEventDetectors.put(ped.getEventDetectorKey(), ped);
        }
    }

    public void removePointEventDetector(String pointEventDetectorKey) {
        synchronized (data.simpleEventDetectors) {
            SimpleEventDetector sed = data.simpleEventDetectors.remove(pointEventDetectorKey);
            if (sed != null)
                sed.terminate();
        }
    }
    ...
    public SimpleEventDetector getSimpleEventDetector(String key) {
        return data.simpleEventDetectors.get(key);
    }
    ...
    public boolean startCompoundEventDetector(CompoundEventDetectorVO ced) {
        ...
        try {
            rt.initialize();
            data.compoundEventDetectors.put(ced.getId(), rt);
            return true;
        }
    ...
    public void stopCompoundEventDetector(int compoundEventDetectorId) {
        CompoundEventDetectorRT rt = data.compoundEventDetectors.remove(compoundEventDetectorId);
        if (rt != null)
            rt.terminate();
    }
    ...
    private PublisherRT<?> getRunningPublisher(int publisherId) {
        for (PublisherRT<?> publisher : data.runningPublishers) {
    ...
    private void startPublisher(PublisherVO<? extends PublishedPointVO> vo) {
        synchronized (data.runningPublishers) {
            ...
            data.runningPublishers.add(publisher);
        }
    }
    ...
    private void stopPublisher(int id) {
        synchronized (data.runningPublishers) {
            ...
            data.runningPublishers.remove(publisher);
            publisher.terminate();
    ...
    private void stopPointLink(int id) {
        synchronized (data.pointLinks) {
            ...
            data.pointLinks.remove(pointLink);
            pointLink.terminate();
        }
    }
    ...
    public MaintenanceEventRT getRunningMaintenanceEvent(int id) {
        for (MaintenanceEventRT rt : data.maintenanceEvents) {
    ...
    private void startMaintenanceEvent(MaintenanceEventVO vo) {
        synchronized (data.maintenanceEvents) {
    ...
    private void stopMaintenanceEvent(int id) {
        synchronized (data.maintenanceEvents) {
            ...
            data.maintenanceEvents.remove(rt);
            rt.terminate();
        }
    }
}
