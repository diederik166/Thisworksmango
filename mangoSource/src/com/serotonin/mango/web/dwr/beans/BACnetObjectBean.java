...
public class BACnetObjectBean {
	...
	@MethodFilter
	public DataPointVO addBacnetPoint(String ip, int port, int networkNumber, String networkAddress, int deviceInstanceNumber, DataSourceEditDwr dataSourceEditDwr) {
	    DataPointVO dp = dataSourceEditDwr.getPoint(Common.NEW_ID, null);
	    BACnetIPPointLocatorVO locator = dp.getPointLocator();
	
	    dp.setName(getObjectName());
	    ...
	    locator.setObjectTypeId(getObjectTypeId());
	    locator.setObjectInstanceNumber(getInstanceNumber());
	    locator.setPropertyIdentifierId(PropertyIdentifier.presentValue.intValue());
	    locator.setDataTypeId(getDataTypeId());
	    locator.setUseCovSubscription(isCov());
